...
const updateGame = [
  ...
  asyncHandler(async (req, res) => {
    ...
    const targetChar = gameData.gameChars.find(
      (char) => char.id === formData.char,
    );
    if (!targetChar) {
    ...
    const checkCoords =
      formData.coordX <= targetChar.coordXMax &&
      formData.coordX >= targetChar.coordXMin &&
      formData.coordY <= targetChar.coordYMax &&
      formData.coordY >= targetChar.coordYMin;
...
export { createGame, updateGame, gameCharacters };
